...
class MainActivity : AppCompatActivity() {
    ...
    private fun setupObservable() {
        ...
        viewModelRetrofit.testUIDataSate.observe(this) {
            val dataState = it ?: return@observe
            binding.progressBar.visibility = if(dataState.isLoading) View.VISIBLE else View.INVISIBLE
            dataState.result?.let { result->
                binding.tv.text = "Receive data from Internet successfully"
                Log.d("fjdla","$result")
            }
            dataState.error?.let {
                Log.d("fjdla error",it )
            }
        }
    }
}